...
class YUVBlock(nn.Module):
    def __init__(self, num_feat=64, num_grow_ch=32):
        self.conv = nn.Conv2d(num_feat, num_grow_ch, 3, 1, 1)
        self.Yconv = nn.Conv2d(num_feat, num_grow_ch, 5, 1, 2)
        self.UVconv = nn.Conv2d(num_feat, num_grow_ch, 3, 1, 1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

        # initialization
        default_init_weights(
            [self.Yconv, self.UVconv], 0.1
        )

    def forward(self,x):
        y, uv = x.shape()
        x1 = self.lrelu(self.Yconv(y))
        x2 = self.lrelu(self.UVconv(uv))

        x3 = self.lrelu(self.conv(torch.cat((x1, x2), 1)))

        # Emperically, we use 0.2 to scale the residual for better performance
        return x3 * 0.2 + x

class YUVBlock_other(nn.Module):
    def __init__(self, num_feat=64, num_grow_ch=32):
        self.Conv1 = nn.Conv2d(num_feat, num_grow_ch, 3, 3, 1)
        self.Conv2 = nn.Conv2d(num_feat, num_grow_ch, 3, 3, 1)
        self.Conv3 = nn.Conv2d(num_feat, num_grow_ch, 3, 3, 1)
