...
from models import Generator, Generator_teacher, MultiscaleDiscriminator, Discriminator_PatchGAN
...
def gan_trainer(cfg, gpu, loader, generator, discriminator):
    ...
    if os.path.exists(cfg.training.GAN.resume):
        ...
        generator.load_state_dict(torch.load("/workspace/BackgroundSR_ESP/outputs/train/2022-08-03/10-28-48/PSNR_cpkt/400000.pth")["g"])
...
def psnr_trainer(cfg, gpu, loader, generator):
    ...
    student_loss = nn.L1Loss().to(gpu)
    teacher_loss = nn.L1Loss().to(gpu)
    psnr_optim = torch.optim.Adam(generator.parameters(), cfg.training.PSNR.lr, (0.9, 0.99))

    teacher_generator = Generator_teacher().to(gpu)
    teacher_generator.load_state_dict(torch.load("/workspace/BackgroundSR_ESP/2022_04_0113_41_59.pth")["g"])
    ...
    for i in range(start_iters, cfg.training.PSNR.n_iters):        
        ...
        preds = generator(lr)
        teacher = teacher_generator(lr)

        s_loss = student_loss(preds, hr) * 0.5
        t_loss = teacher_loss(preds,teacher) * 0.5

        loss_sum = s_loss + t_loss

        loss_dict["l1loss"] = loss_sum
        ...
        if gpu == 0:
            ...
            results = torch.cat((hr, lr, preds, teacher), 2)
                    ...
                    wandb.log(
                        {
                            ...
                            "GT": [wandb.Image(hr, caption="step{}_Label".format(i))],
                            "Preds": [wandb.Image(preds, caption="step{}_Label".format(i))],
                            "teacher": [wandb.Image(teacher, caption="step{}_Label".format(i))],
                            "LQ":[wandb.Image(lr, caption="step{}_Label".format(i))]
                        }
                    )
                else:
